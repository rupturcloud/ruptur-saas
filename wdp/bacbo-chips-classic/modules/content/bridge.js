...
(function () {
  ...
  chrome.runtime.onMessage.addListener((message, _sender, sendResponse) => {
    ...
    // ── Executor Humanizado (novo motor) ──────────────────────────────────
    if (type === 'EXECUTE_BET_HUMANIZED') {
      const executor = window.__BETIA.executor;
      if (!executor) {
        sendResponse({ ok: false, motivo: 'Executor humanizado não carregado.' });
        return true;
      }
      const acao = String(message.acao || message.target || '').charAt(0).toUpperCase();
      const stake = Number(message.stake || message.amount || 0);
      const protecao = message.protecao !== false;
      executor
        .realizarAposta(acao, stake, { protecao })
        .then(sendResponse)
        .catch((err) => sendResponse({ ok: false, motivo: err?.message || 'Erro inesperado.' }));
      return true;
    }
    ...
    if (type === 'EXECUTOR_STATUS') {
      ...
      sendResponse({
        ...
        falhas: executor?.getFalhas?.() || [],
        fallbackState: executor?.fallbackState?.() || null,
      });
      return true;
    }

    if (type === 'EXECUTOR_RESET_FALHAS') {
      window.__BETIA.executor?.resetarFalhas?.();
      sendResponse({ ok: true });
      return true;
    }
