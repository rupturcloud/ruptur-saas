...
import { crmApi } from '../../api/crm.api.js';
...
export default function Crm() {
  ...
  const dragRef = useRef(null); // { chatId, fromStatus }
  ...
  const loadPipeline = useCallback(async (key) => {
    if (!key) return;
    setLoading(true);
    setError('');
    try {
      const data = await crmApi.getPipeline(key);
  ...
  useEffect(() => { if (instanceKey) loadPipeline(instanceKey); }, [instanceKey, loadPipeline]);

  // ── Drag-and-drop ──
  const onDragStart = (card, fromStatus) => (e) => {
    dragRef.current = { chatId: card.chatId, fromStatus, sourceInstance: card._sourceInstance };
    e.dataTransfer.effectAllowed = 'move';
    try { e.dataTransfer.setData('text/plain', card.chatId); } catch { /* noop */ }
  };
  ...
  const onDropColumn = (targetStage) => async (e) => {
    ...
    const next = columns.map((col) => {
      if (col.lead_status === drag.fromStatus) {
        const idx = col.cards.findIndex((c) => c.chatId === drag.chatId);
        if (idx >= 0) { movedCard = { ...col.cards[idx], leadStatus: targetStage.lead_status }; }
        return { ...col, cards: col.cards.filter((c) => c.chatId !== drag.chatId) };
      }
      return col;
    ...
    try {
      const order = Date.now(); // ordem simples: mais recente no topo
      const targetInstanceKey = drag.sourceInstance || instanceKey;
      await crmApi.moveCard(targetInstanceKey, drag.chatId, targetStage.lead_status, order);
    } catch (err) {
  ...
  const saveLead = async (patch) => {
    ...
    setSavingLead(true);
    try {
      const targetInstanceKey = selected._sourceInstance || instanceKey;
      await crmApi.updateCard(targetInstanceKey, selected.chatId, patch);
      setSelected((s) => ({ ...s, ...mapPatchToCard(patch) }));
      loadPipeline(instanceKey);
    } catch (e) {
      setError(e.message);
    } finally {
      setSavingLead(false);
