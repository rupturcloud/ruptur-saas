...
// ════════════════════════════════════════════════════════════════════
// WILL SERVER V7 INTEGRATION
// ════════════════════════════════════════════════════════════════════

const ligarBtn = document.querySelector('[id*="ligar"], [class*="ligar"], button:contains("LIGAR")') || 
                document.querySelector('button') || null;

// Cria botão LIGAR se não existir
let willServerBtn = document.querySelector('[id="will-server-btn"]');
if (!willServerBtn) {
  willServerBtn = document.createElement('button');
  willServerBtn.id = 'will-server-btn';
  willServerBtn.textContent = '🤖 LIGAR';
  willServerBtn.style.cssText = `
    padding: 10px 20px;
    background: #00aa44;
    color: white;
    border: none;
  ...
  container.appendChild(willServerBtn);
}

let robotRunning = false;

willServerBtn.addEventListener('click', async () => {
  if (!robotRunning) {
    willServerBtn.textContent = '⏹ PARANDO...';
    willServerBtn.disabled = true;

    try {
      const response = await chrome.runtime.sendMessage({ type: 'ROBOT_START' });
      if (response?.ok) {
        robotRunning = true;
        willServerBtn.textContent = '⏹ PARAR';
        willServerBtn.style.background = '#dd4444';
        console.log('[Will Server] 🚀 Robô iniciado');
      } else {
        console.error('[Will Server] Erro:', response?.error);
        willServerBtn.textContent = '🤖 LIGAR';
      }
    } catch (e) {
      console.error('[Will Server] Erro:', e);
      willServerBtn.textContent = '🤖 LIGAR';
    } finally {
      willServerBtn.disabled = false;
    }
  } else {
    willServerBtn.textContent = '🤖 LIGANDO...';
    willServerBtn.disabled = true;

    try {
      const response = await chrome.runtime.sendMessage({ type: 'ROBOT_STOP' });
      if (response?.ok) {
        robotRunning = false;
        willServerBtn.textContent = '🤖 LIGAR';
        willServerBtn.style.background = '#00aa44';
...
// Monitora estado do servidor
setInterval(async () => {
  try {
    const response = await chrome.runtime.sendMessage({ type: 'ROBOT_STATE' });
...
console.log('[Will Server] Integration loaded');
